import bg from "../assets/images/funky-lines.webp";
import img from "../assets/images/489009076_9521894934603823_7702292921294509636_n.jpg";
import { Link, NavLink } from "react-router-dom";
import { Outlet } from "react-router-dom";
import Footer from "../components/layout/Footer";
import Header from "../components/layout/Header";

const sidebarData = [
  {
    title: "فيديو الشرح",
    btn: "فيديو الشرح",
    path: "/media",
    icon: (
      <svg
        version="1.1"
        id="Layer_1"
        className="w-7"
        xmlns="http://www.w3.org/2000/svg"
        xmlnsXlink="http://www.w3.org/1999/xlink"
        viewBox="0 0 392.663 392.663"
        xmlSpace="preserve"
      >
        <path
          style={{ fill: "#ffffff" }}
          d="M21.786,342.368c0,5.301,4.331,9.632,9.632,9.632h247.725c5.301,0,9.632-4.331,9.632-9.632V197.107
    c0-5.301-4.331-9.632-9.632-9.632H31.547c-5.301,0-9.632,4.331-9.632,9.632"
        />
        <rect
          x="43.572"
          y="209.261"
          style={{ fill: "#ffc10d" }}
          width="223.483"
          height="121.083"
        />
        <polygon
          style={{ fill: "#56ace0" }}
          points="310.691,296.663 370.747,324.202 370.747,215.273 310.691,242.877 "
        />
        <path
          style={{ fill: "#194f82" }}
          d="M377.083,188.38l-66.392,30.448v-21.721c0-17.39-14.158-31.547-31.547-31.547H31.547
    C14.158,165.56,0,179.717,0,197.107v145.261c0,17.39,14.158,31.547,31.547,31.547h247.725c17.39,0,31.547-14.158,31.547-31.547
    v-21.786l66.392,30.448c7.176,3.297,15.451-1.939,15.451-9.891V198.271C392.533,190.384,384.259,185.018,377.083,188.38z
     M370.747,324.202l-60.057-27.539v-53.786l60.057-27.604V324.202z M288.905,342.368c0,5.301-4.331,9.632-9.632,9.632H31.547
    c-5.301,0-9.632-4.331-9.632-9.632v-39.952h21.786c6.012,0,10.925-4.849,10.925-10.925c0-6.012-4.848-10.925-10.925-10.925H21.786
    V258.78h43.636c6.012,0,10.925-4.848,10.925-10.925c0-6.077-4.848-10.796-10.925-10.796H21.786v-39.952
    c0-5.301,4.331-9.632,9.632-9.632h247.725c5.301,0,9.632,4.331,9.632,9.632v145.261H288.905z"
        />
        <path
          style={{ fill: "#56ace0" }}
          d="M121.341,82.489c0-23.079-18.747-41.891-41.891-41.891S37.56,59.345,37.56,82.489
    s18.747,41.891,41.891,41.891C102.465,124.38,121.341,105.632,121.341,82.489z"
        />
        <path
          style={{ fill: "#ffffff" }}
          d="M273.261,82.489c0-23.079-18.747-41.891-41.891-41.891s-41.891,18.747-41.891,41.891
    s18.747,41.891,41.891,41.891S273.261,105.632,273.261,82.489z"
        />
        <g>
          <path
            style={{ fill: "#194f82" }}
            d="M79.321,18.748c-35.168,0-63.741,28.574-63.741,63.741s28.574,63.741,63.741,63.741
        s63.741-28.574,63.741-63.741S114.489,18.748,79.321,18.748z M37.495,82.489c0-23.079,18.747-41.891,41.891-41.891
        s41.891,18.747,41.891,41.891s-18.747,41.891-41.891,41.891S37.495,105.632,37.495,82.489z"
          />
          <path
            style={{ fill: "#194f82" }}
            d="M231.37,18.748c-35.168,0-63.741,28.574-63.741,63.741s28.574,63.741,63.741,63.741
        s63.741-28.574,63.741-63.741S266.408,18.748,231.37,18.748z M189.414,82.489c0-23.079,18.747-41.891,41.891-41.891
        s41.891,18.747,41.891,41.891s-18.747,41.891-41.891,41.891C208.226,124.38,189.414,105.632,189.414,82.489z"
          />
        </g>
      </svg>
    ),
  },
  {
    title: "فيديو الواجب",
    btn: "فيديو الواجب",
    path: "/media/homework",

    icon: (
      <svg
        version="1.1"
        id="Layer_1"
        className="w-7"
        xmlns="http://www.w3.org/2000/svg"
        xmlnsXlink="http://www.w3.org/1999/xlink"
        viewBox="0 0 392.663 392.663"
        xmlSpace="preserve"
      >
        <path
          style={{ fill: "#ffffff" }}
          d="M21.786,342.368c0,5.301,4.331,9.632,9.632,9.632h247.725c5.301,0,9.632-4.331,9.632-9.632V197.107
    c0-5.301-4.331-9.632-9.632-9.632H31.547c-5.301,0-9.632,4.331-9.632,9.632"
        />
        <rect
          x="43.572"
          y="209.261"
          style={{ fill: "#ffc10d" }}
          width="223.483"
          height="121.083"
        />
        <polygon
          style={{ fill: "#56ace0" }}
          points="310.691,296.663 370.747,324.202 370.747,215.273 310.691,242.877 "
        />
        <path
          style={{ fill: "#194f82" }}
          d="M377.083,188.38l-66.392,30.448v-21.721c0-17.39-14.158-31.547-31.547-31.547H31.547
    C14.158,165.56,0,179.717,0,197.107v145.261c0,17.39,14.158,31.547,31.547,31.547h247.725c17.39,0,31.547-14.158,31.547-31.547
    v-21.786l66.392,30.448c7.176,3.297,15.451-1.939,15.451-9.891V198.271C392.533,190.384,384.259,185.018,377.083,188.38z
     M370.747,324.202l-60.057-27.539v-53.786l60.057-27.604V324.202z M288.905,342.368c0,5.301-4.331,9.632-9.632,9.632H31.547
    c-5.301,0-9.632-4.331-9.632-9.632v-39.952h21.786c6.012,0,10.925-4.849,10.925-10.925c0-6.012-4.848-10.925-10.925-10.925H21.786
    V258.78h43.636c6.012,0,10.925-4.848,10.925-10.925c0-6.077-4.848-10.796-10.925-10.796H21.786v-39.952
    c0-5.301,4.331-9.632,9.632-9.632h247.725c5.301,0,9.632,4.331,9.632,9.632v145.261H288.905z"
        />
        <path
          style={{ fill: "#56ace0" }}
          d="M121.341,82.489c0-23.079-18.747-41.891-41.891-41.891S37.56,59.345,37.56,82.489
    s18.747,41.891,41.891,41.891C102.465,124.38,121.341,105.632,121.341,82.489z"
        />
        <path
          style={{ fill: "#ffffff" }}
          d="M273.261,82.489c0-23.079-18.747-41.891-41.891-41.891s-41.891,18.747-41.891,41.891
    s18.747,41.891,41.891,41.891S273.261,105.632,273.261,82.489z"
        />
        <g>
          <path
            style={{ fill: "#194f82" }}
            d="M79.321,18.748c-35.168,0-63.741,28.574-63.741,63.741s28.574,63.741,63.741,63.741
        s63.741-28.574,63.741-63.741S114.489,18.748,79.321,18.748z M37.495,82.489c0-23.079,18.747-41.891,41.891-41.891
        s41.891,18.747,41.891,41.891s-18.747,41.891-41.891,41.891S37.495,105.632,37.495,82.489z"
          />
          <path
            style={{ fill: "#194f82" }}
            d="M231.37,18.748c-35.168,0-63.741,28.574-63.741,63.741s28.574,63.741,63.741,63.741
        s63.741-28.574,63.741-63.741S266.408,18.748,231.37,18.748z M189.414,82.489c0-23.079,18.747-41.891,41.891-41.891
        s41.891,18.747,41.891,41.891s-18.747,41.891-41.891,41.891C208.226,124.38,189.414,105.632,189.414,82.489z"
          />
        </g>
      </svg>
    ),
  },
  {
    title: "امتحان الحصة",
    btn: "امتحان سريع",
    path: "/media/lesson-exam",

    icon: (
      <svg
        version="1.1"
        id="Layer_1"
        className="w-7"
        xmlns="http://www.w3.org/2000/svg"
        xmlnsXlink="http://www.w3.org/1999/xlink"
        viewBox="0 0 512 512"
        xmlSpace="preserve"
      >
        <path
          style={{ fill: "#509fe8" }}
          d="M427.023,503.983H84.977c-9.446,0-17.102-7.656-17.102-17.102V84.977
    c0-9.446,7.656-17.102,17.102-17.102h342.046c9.446,0,17.102,7.656,17.102,17.102v401.904
    C444.125,496.327,436.469,503.983,427.023,503.983z"
        />
        <path
          style={{ fill: "#4d8ccf" }}
          d="M84.977,486.881V84.977c0-9.445,7.656-17.102,17.102-17.102H84.977
    c-9.446,0-17.102,7.658-17.102,17.102v401.904c0,9.445,7.656,17.102,17.102,17.102h17.102
    C92.634,503.983,84.977,496.326,84.977,486.881z"
        />
        <rect
          x="110.63"
          y="67.875"
          style={{ fill: "#f6f6f7" }}
          width="290.739"
          height="393.353"
        />
        <path
          style={{ fill: "#d9dcdf" }}
          d="M152.852,110.63h206.296c4.427,0,8.017-3.589,8.017-8.017V67.875h-222.33v34.739
    C144.835,107.041,148.424,110.63,152.852,110.63z"
        />
        <path
          style={{ fill: "#ffc44f" }}
          d="M350.063,93.528V59.324c0-9.446-7.656-17.102-17.102-17.102h-42.756
    c0-18.891-15.314-34.205-34.205-34.205s-34.205,15.314-34.205,34.205H179.04c-9.446,0-17.102,7.656-17.102,17.102v34.205H350.063z"
        />
        <path
          style={{ fill: "#ffd791" }}
          d="M332.96,42.221h-17.102c9.446,0,17.102,7.658,17.102,17.102v34.205h17.102V59.324
    C350.063,49.879,342.406,42.221,332.96,42.221z"
        />
        <path
          style={{ fill: "#f7ab49" }}
          d="M196.142,42.221H179.04c-9.446,0-17.102,7.658-17.102,17.102v34.205h17.102V59.324
    C179.04,49.879,186.696,42.221,196.142,42.221z"
        />
        <path
          style={{ fill: "#ff8c78" }}
          d="M356.808,316.761l-85.512-171.023c-2.897-5.794-8.818-9.454-15.297-9.454
    c-6.479,0-12.4,3.66-15.297,9.454l-85.511,171.023c-4.224,8.447-0.8,18.721,7.649,22.945c8.445,4.223,18.721,0.799,22.945-7.649
    l33.754-67.506h72.923l33.754,67.506c2.996,5.993,9.035,9.459,15.31,9.458c2.57,0,5.18-0.581,7.635-1.809
    C357.608,335.482,361.033,325.209,356.808,316.761z M236.64,230.347L256,191.628l19.36,38.719H236.64z"
        />
        <path
          d="M427.023,59.858h-68.944v-0.534c0-13.851-11.268-25.119-25.119-25.119h-35.502C293.703,14.743,276.542,0,256,0
    c-20.541,0-37.703,14.743-41.458,34.205H179.04c-13.851,0-25.119,11.268-25.119,25.119v0.534H84.977
    c-13.851,0-25.119,11.268-25.119,25.119v273.637c0,4.427,3.589,8.017,8.017,8.017s8.017-3.589,8.017-8.017V84.977
    c0-5.01,4.076-9.086,9.086-9.086h17.637v385.336c0,4.427,3.589,8.017,8.017,8.017H401.37c4.428,0,8.017-3.589,8.017-8.017v-299.29
    c0-4.427-3.588-8.017-8.017-8.017s-8.017,3.589-8.017,8.017v291.273H118.647V75.891h35.273v17.637c0,4.427,3.589,8.017,8.017,8.017
    h188.125c4.428,0,8.017-3.589,8.017-8.017V75.891h35.273v43.29c0,4.427,3.588,8.017,8.017,8.017s8.017-3.589,8.017-8.017v-43.29
    h17.637c5.01,0,9.086,4.076,9.086,9.086v401.904c0,5.01-4.076,9.086-9.086,9.086H84.977c-5.01,0-9.086-4.076-9.086-9.086V401.37
    c0-4.427-3.589-8.017-8.017-8.017s-8.017,3.589-8.017,8.017v85.511c0,13.851,11.268,25.119,25.119,25.119h342.046
    c13.851,0,25.119-11.268,25.119-25.119V84.977C452.142,71.126,440.874,59.858,427.023,59.858z M342.046,85.511H169.954V59.324
    c0-5.01,4.076-9.086,9.086-9.086h42.756c4.427,0,8.017-3.589,8.017-8.017c0-14.44,11.748-26.188,26.188-26.188
    c14.441,0,26.188,11.748,26.188,26.188c0,4.427,3.588,8.017,8.017,8.017h42.756c5.01,0,9.086,4.076,9.086,9.086V85.511z"
        />
        <path
          d="M256,25.653c-4.427,0-8.017,3.589-8.017,8.017v8.551c0,4.427,3.589,8.017,8.017,8.017c4.428,0,8.017-3.589,8.017-8.017
    V33.67C264.017,29.243,260.428,25.653,256,25.653z"
        />
        <path
          d="M352.746,346.876c12.388-6.195,17.427-21.313,11.233-33.7l-85.511-171.023c-4.282-8.565-12.891-13.886-22.467-13.886
    c-9.575,0-18.185,5.321-22.467,13.886l-85.511,171.023c-6.194,12.387-1.155,27.506,11.234,33.7
    c12.388,6.194,27.506,1.155,33.7-11.234l31.538-63.074h63.014l31.537,63.074C325.17,347.896,340.458,353.017,352.746,346.876z
     M333.385,328.472l-33.754-67.506c-1.359-2.716-4.133-4.432-7.17-4.432h-72.923c-3.037,0-5.813,1.716-7.17,4.432l-33.754,67.506
    c-2.241,4.482-7.71,6.303-12.19,4.064c-4.481-2.24-6.304-7.709-4.064-12.19l85.511-171.023c3.286-6.572,12.968-6.572,16.254,0
    l85.512,171.023l0,0c2.288,4.575,0.231,10.263-4.412,12.355C340.831,334.681,335.533,332.768,333.385,328.472z"
        />
        <path
          d="M263.17,188.043c-2.897-5.792-11.444-5.794-14.34,0l-19.359,38.719c-2.621,5.24,1.323,11.602,7.17,11.602h38.719
    c5.847,0,9.792-6.362,7.17-11.602L263.17,188.043z M249.612,222.33L256,209.553l6.389,12.776H249.612z"
        />
        <path
          d="M332.96,128.267c-4.428,0-8.017,3.589-8.017,8.017v9.086h-9.086c-4.428,0-8.017,3.589-8.017,8.017
    c0,4.427,3.588,8.017,8.017,8.017h9.086v9.086c0,4.427,3.588,8.017,8.017,8.017c4.428,0,8.017-3.589,8.017-8.017v-9.086h9.086
    c4.428,0,8.017-3.589,8.017-8.017c0-4.427-3.588-8.017-8.017-8.017h-9.086v-9.086C340.977,131.857,337.389,128.267,332.96,128.267z"
        />
        <path
          d="M332.426,384.267c0-4.427-3.588-8.017-8.017-8.017H187.591c-4.427,0-8.017,3.589-8.017,8.017
    c0,4.427,3.589,8.017,8.017,8.017h136.818C328.838,392.284,332.426,388.695,332.426,384.267z"
        />
        <path
          d="M187.591,410.455c-4.427,0-8.017,3.589-8.017,8.017c0,4.427,3.589,8.017,8.017,8.017h76.96c4.428,0,8.017-3.589,8.017-8.017
    c0-4.427-3.588-8.017-8.017-8.017H187.591z"
        />
      </svg>
    ),
  },
  {
    title: "ملف الحصة",
    btn: "ملف الشرح",
    path: "/media/lesson-file",

    icon: (
      <svg
        version="1.1"
        id="Layer_1"
        xmlns="http://www.w3.org/2000/svg"
        xmlnsXlink="http://www.w3.org/1999/xlink"
        className="w-7"
        viewBox="-50.13 -50.13 601.59 601.59"
        xmlSpace="preserve"
        fill="#000000"
        stroke="#000000"
        strokeWidth="3.0079979999999997"
      >
        <g id="SVGRepo_bgCarrier" strokeWidth="0"></g>
        <g
          id="SVGRepo_tracerCarrier"
          strokeLinecap="round"
          strokeLinejoin="round"
          stroke="#81c7fd"
          strokeWidth="18.047988"
        >
          <g>
            <path
              style={{ fill: "#2fc0e9" }}
              d="M452.8,56.533c6.4-6.4,9.6-14.933,9.6-23.467S459.2,16,452.8,9.6c-12.8-12.8-34.133-12.8-46.933,0 L363.2,52.267l10.667,4.267L414.4,16c8.533-8.533,23.467-8.533,32,0c4.267,4.267,6.4,9.6,6.4,16s-2.133,11.733-6.4,16 l-27.733,27.733l17.067-2.133L452.8,56.533z"
            />
            <path
              style={{ fill: "#2fc0e9" }}
              d="M318.4,97.067c-17.067,17.067-17.067,44.8,0,61.867c8.533,8.533,20.267,12.8,30.933,12.8 c11.733,0,22.4-4.267,30.933-12.8L443.2,97.067l-7.467-7.467L372.8,152.533c-12.8,12.8-34.133,12.8-46.933,0 c-12.8-12.8-12.8-34.133,0-46.933L372.8,58.667L362.133,54.4L318.4,97.067z"
            />
          </g>
          <path
            style={{ fill: "#e0e0df" }}
            d="M389.867,46.933H58.133c-10.667,0-19.2,8.533-19.2,19.2v416c0,10.667,8.533,19.2,19.2,19.2h331.733 c10.667,0,19.2-8.533,19.2-19.2v-416C409.067,55.467,400.533,46.933,389.867,46.933z"
          />
          <path
            style={{ fill: "#2fc0e9" }}
            d="M397.333,66.133l-7.467-7.467l-51.2,51.2c-8.533,8.533-8.533,22.4,0,30.933 c8.533,8.533,22.4,8.533,30.933,0l65.067-65.067L417.6,77.867l-55.467,55.467c-4.267,4.267-11.733,4.267-16,0 c-4.267-4.267-4.267-11.733,0-16L397.333,66.133z"
          />
          <g>
            <rect
              x="117.867"
              y="178.133"
              style={{ fill: "#b6bcbc" }}
              width="212.267"
              height="21.333"
            />
            <rect
              x="117.867"
              y="332.8"
              style={{ fill: "#b6bcbc" }}
              width="212.267"
              height="21.333"
            />
            <rect
              x="117.867"
              y="410.667"
              style={{ fill: "#b6bcbc" }}
              width="212.267"
              height="21.333"
            />
            <rect
              x="117.867"
              y="256"
              style={{ fill: "#b6bcbc" }}
              width="212.267"
              height="21.333"
            />
          </g>
        </g>
        <g id="SVGRepo_iconCarrier">
          <g>
            <path
              style={{ fill: "#2fc0e9" }}
              d="M452.8,56.533c6.4-6.4,9.6-14.933,9.6-23.467S459.2,16,452.8,9.6c-12.8-12.8-34.133-12.8-46.933,0 L363.2,52.267l10.667,4.267L414.4,16c8.533-8.533,23.467-8.533,32,0c4.267,4.267,6.4,9.6,6.4,16s-2.133,11.733-6.4,16 l-27.733,27.733l17.067-2.133L452.8,56.533z"
            />
            <path
              style={{ fill: "#2fc0e9" }}
              d="M318.4,97.067c-17.067,17.067-17.067,44.8,0,61.867c8.533,8.533,20.267,12.8,30.933,12.8 c11.733,0,22.4-4.267,30.933-12.8L443.2,97.067l-7.467-7.467L372.8,152.533c-12.8,12.8-34.133,12.8-46.933,0 c-12.8-12.8-12.8-34.133,0-46.933L372.8,58.667L362.133,54.4L318.4,97.067z"
            />
          </g>
          <path
            style={{ fill: "#e0e0df" }}
            d="M389.867,46.933H58.133c-10.667,0-19.2,8.533-19.2,19.2v416c0,10.667,8.533,19.2,19.2,19.2h331.733 c10.667,0,19.2-8.533,19.2-19.2v-416C409.067,55.467,400.533,46.933,389.867,46.933z"
          />
          <path
            style={{ fill: "#2fc0e9" }}
            d="M397.333,66.133l-7.467-7.467l-51.2,51.2c-8.533,8.533-8.533,22.4,0,30.933 c8.533,8.533,22.4,8.533,30.933,0l65.067-65.067L417.6,77.867l-55.467,55.467c-4.267,4.267-11.733,4.267-16,0 c-4.267-4.267-4.267-11.733,0-16L397.333,66.133z"
          />
          <g>
            <rect
              x="117.867"
              y="178.133"
              style={{ fill: "#b6bcbc" }}
              width="212.267"
              height="21.333"
            />
            <rect
              x="117.867"
              y="332.8"
              style={{ fill: "#b6bcbc" }}
              width="212.267"
              height="21.333"
            />
            <rect
              x="117.867"
              y="410.667"
              style={{ fill: "#b6bcbc" }}
              width="212.267"
              height="21.333"
            />
            <rect
              x="117.867"
              y="256"
              style={{ fill: "#b6bcbc" }}
              width="212.267"
              height="21.333"
            />
          </g>
        </g>
      </svg>
    ),
  },
];
function MediaPreview() {
  return (
    <>
      <Header />
      <div
        className="sm:mx-6 my-6 py-6 rounded-2xl"
        style={{ background: `url(${bg})` }}
      >
        <section
          id="background"
          className="rounded-md lg:px-28 px-8 mb-20 mx-1 sm:mx-4 shadow-2xl text-slate-100 relative bg-sky-400"
        >
          <div className="flex flex-col lg:flex-row py-6 px-0 sm:px-6 justify-center text-center gap-x-5">
            <div className="text-white md:pt-16 w-full lg:w-1/2">
              <div className="mb-5 sm:mb-10">
                <div className="flex justify-center gap-3">
                  <div className="flex items-center">
                    <svg
                      xmlns="http://www.w3.org/2000/svg"
                      width="24"
                      height="24"
                      viewBox="0 0 24 24"
                    >
                      <path
                        fill="currentColor"
                        d="M20.27,4.74a4.93,4.93,0,0,1,1.52,4.61,5.32,5.32,0,0,1-4.1,4.51,5.12,5.12,0,0,1-5.2-1.5,5.53,5.53,0,0,0,6.13-1.48A5.66,5.66,0,0,0,20.27,4.74ZM12.32,11.53a5.49,5.49,0,0,0-1.47-6.2A5.57,5.57,0,0,0,4.71,3.72,5.17,5.17,0,0,1,9.53,2.2,5.52,5.52,0,0,1,13.9,6.45,5.28,5.28,0,0,1,12.32,11.53ZM19.2,20.29a4.92,4.92,0,0,1-4.72,1.49,5.32,5.32,0,0,1-4.34-4.05A5.2,5.2,0,0,1,11.6,12.5a5.6,5.6,0,0,0,1.51,6.13A5.63,5.63,0,0,0,19.2,20.29ZM3.79,19.38A5.18,5.18,0,0,1,2.32,14a5.3,5.3,0,0,1,4.59-4,5,5,0,0,1,4.58,1.61,5.55,5.55,0,0,0-6.32,1.69A5.46,5.46,0,0,0,3.79,19.38ZM12.23,12a5.11,5.11,0,0,0,3.66-5,5.75,5.75,0,0,0-3.18-6,5,5,0,0,1,4.42,2.3,5.21,5.21,0,0,1,.24,5.92A5.4,5.4,0,0,1,12.23,12ZM11.76,12a5.18,5.18,0,0,0-3.68,5.09,5.58,5.58,0,0,0,3.19,5.79c-1,.35-2.9-.46-4-1.68A5.51,5.51,0,0,1,11.76,12ZM23,12.63a5.07,5.07,0,0,1-2.35,4.52,5.23,5.23,0,0,1-5.91.2,5.24,5.24,0,0,1-2.67-4.77,5.51,5.51,0,0,0,5.45,3.33A5.52,5.52,0,0,0,23,12.63ZM1,11.23a5,5,0,0,1,2.49-4.5,5.23,5.23,0,0,1,5.81-.06,5.3,5.3,0,0,1,2.61,4.74A5.56,5.56,0,0,0,6.56,8.06,5.71,5.71,0,0,0,1,11.23Z"
                      >
                        <animateTransform
                          attributeName="transform"
                          dur="1.5s"
                          repeatCount="indefinite"
                          type="rotate"
                          values="0 12 12;360 12 12"
                        />
                      </path>
                    </svg>
                  </div>

                  <div className="text-center font-semibold text-3xl font-aqsa text-[var(--primary)] sm:text-5xl">
                    محتوي المحاضرة
                  </div>

                  <div className="flex items-center">
                    <svg
                      xmlns="http://www.w3.org/2000/svg"
                      width="24"
                      height="24"
                      viewBox="0 0 24 24"
                    >
                      <path
                        fill="currentColor"
                        d="M20.27,4.74a4.93,4.93,0,0,1,1.52,4.61,5.32,5.32,0,0,1-4.1,4.51,5.12,5.12,0,0,1-5.2-1.5,5.53,5.53,0,0,0,6.13-1.48A5.66,5.66,0,0,0,20.27,4.74ZM12.32,11.53a5.49,5.49,0,0,0-1.47-6.2A5.57,5.57,0,0,0,4.71,3.72,5.17,5.17,0,0,1,9.53,2.2,5.52,5.52,0,0,1,13.9,6.45,5.28,5.28,0,0,1,12.32,11.53ZM19.2,20.29a4.92,4.92,0,0,1-4.72,1.49,5.32,5.32,0,0,1-4.34-4.05A5.2,5.2,0,0,1,11.6,12.5a5.6,5.6,0,0,0,1.51,6.13A5.63,5.63,0,0,0,19.2,20.29ZM3.79,19.38A5.18,5.18,0,0,1,2.32,14a5.3,5.3,0,0,1,4.59-4,5,5,0,0,1,4.58,1.61,5.55,5.55,0,0,0-6.32,1.69A5.46,5.46,0,0,0,3.79,19.38ZM12.23,12a5.11,5.11,0,0,0,3.66-5,5.75,5.75,0,0,0-3.18-6,5,5,0,0,1,4.42,2.3,5.21,5.21,0,0,1,.24,5.92A5.4,5.4,0,0,1,12.23,12ZM11.76,12a5.18,5.18,0,0,0-3.68,5.09,5.58,5.58,0,0,0,3.19,5.79c-1,.35-2.9-.46-4-1.68A5.51,5.51,0,0,1,11.76,12ZM23,12.63a5.07,5.07,0,0,1-2.35,4.52,5.23,5.23,0,0,1-5.91.2,5.24,5.24,0,0,1-2.67-4.77,5.51,5.51,0,0,0,5.45,3.33A5.52,5.52,0,0,0,23,12.63ZM1,11.23a5,5,0,0,1,2.49-4.5,5.23,5.23,0,0,1,5.81-.06,5.3,5.3,0,0,1,2.61,4.74A5.56,5.56,0,0,0,6.56,8.06,5.71,5.71,0,0,0,1,11.23Z"
                      >
                        <animateTransform
                          attributeName="transform"
                          dur="1.5s"
                          repeatCount="indefinite"
                          type="rotate"
                          values="0 12 12;360 12 12"
                        />
                      </path>
                    </svg>
                  </div>
                </div>
              </div>
              <div className="space-y-6">
                {/* العداد */}
                <div className="flex flex-wrap justify-center gap-4">
                  <span className="bg-blue-600 text-white px-4 py-1 rounded-full text-sm font-bold">
                    📹 316+ فيديوهات
                  </span>
                  <span className="bg-pink-600 text-white px-4 py-1 rounded-full text-sm font-bold">
                    📄 80+ امتحانات
                  </span>
                  <span className="bg-indigo-600 text-white px-4 py-1 rounded-full text-sm font-bold">
                    📘 41+ واجبات
                  </span>
                  <span className="bg-sky-600 text-white px-4 py-1 rounded-full text-sm font-bold">
                    📁 95+ ملفات
                  </span>
                </div>

                {/* العنوان */}
                <h2 className="text-xl md:text-2xl font-bold">
                  الكورس المجاني ليالي الامتحان ( برنامج التفوق ) - (أحمد
                  الهواري - لغة عربية - 3 ثانوي )
                </h2>

                {/* الوصف */}
                <div>
                  •الكورس ده هينزل فيه امتحانات برنامج التفوق عشان تبقا طبقت على
                  الجزء اللي ذاكرته مع المستر 📝و متنساش الامتحانات الاسترشادية
                  مهمة و هتكون موجودة هنا, شد حيلك يا بطل💪❤️•
                </div>

                {/* آخر تحديث */}
                <div className="text-sm mt-6 text-center text-gray-800 bg-white rounded-full inline-block px-4 py-1 shadow">
                  🗓️ الجمعة، 26 يوليو 2024 - آخر تحديث للكورس
                </div>

                {/* الأيقونة الكبيرة */}
                <div className="hidden lg:flex justify-center">
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
                    width="10em"
                    height="10em"
                    viewBox="0 0 512 512"
                  >
                    <path
                      fill="#2a81bf"
                      d="M293.5 298a7.7 7.7 0 0 0-5.3 2L256 330.9L223.8 300a7.8 7.8 0 0 0-10.6 0a6.9 6.9 0 0 0 0 10l37.5 35.8a7.8 7.8 0 0 0 10.6 0l37.5-35.7a6.9 6.9 0 0 0 0-10.1a7.7 7.7 0 0 0-5.3-2.1Z"
                      opacity="0"
                    >
                      <animateTransform
                        attributeName="transform"
                        begin="-0.65s"
                        calcMode="spline"
                        dur="1.5s"
                        keySplines=".55, 0, .1, 1; .55, 0, .1, 1"
                        repeatCount="indefinite"
                        type="translate"
                        values="0 -30; 0 0; 0 30"
                      />
                      <animate
                        attributeName="opacity"
                        begin="-0.65s"
                        calcMode="spline"
                        dur="1.5s"
                        keySplines=".55, 0, .1, 1; .55, 0, .1, 1"
                        repeatCount="indefinite"
                        values="0; 1; 0"
                      />
                    </path>
                    <path
                      fill="#2a81bf"
                      d="M293.5 253a7.7 7.7 0 0 0-5.3 2L256 285.9L223.8 255a7.8 7.8 0 0 0-10.6 0a6.9 6.9 0 0 0 0 10l37.5 35.8a7.8 7.8 0 0 0 10.6 0l37.5-35.7a6.9 6.9 0 0 0 0-10.1a7.7 7.7 0 0 0-5.3-2.1Z"
                      opacity="0"
                    >
                      <animateTransform
                        attributeName="transform"
                        begin="-0.6s"
                        calcMode="spline"
                        dur="1.5s"
                        keySplines=".55, 0, .1, 1; .55, 0, .1, 1"
                        repeatCount="indefinite"
                        type="translate"
                        values="0 -30; 0 0; 0 30"
                      />
                      <animate
                        attributeName="opacity"
                        begin="-0.6s"
                        calcMode="spline"
                        dur="1.5s"
                        keySplines=".55, 0, .1, 1; .55, 0, .1, 1"
                        repeatCount="indefinite"
                        values="0; 1; 0"
                      />
                    </path>
                    <path
                      fill="#2a81bf"
                      d="M293.5 208a7.7 7.7 0 0 0-5.3 2L256 240.9L223.8 210a7.8 7.8 0 0 0-10.6 0a6.9 6.9 0 0 0 0 10l37.5 35.8a7.8 7.8 0 0 0 10.6 0l37.5-35.7a6.9 6.9 0 0 0 0-10.1a7.7 7.7 0 0 0-5.3-2.1Z"
                      opacity="0"
                    >
                      <animateTransform
                        attributeName="transform"
                        begin="-0.55s"
                        calcMode="spline"
                        dur="1.5s"
                        keySplines=".55, 0, .1, 1; .55, 0, .1, 1"
                        repeatCount="indefinite"
                        type="translate"
                        values="0 -30; 0 0; 0 30"
                      />
                      <animate
                        attributeName="opacity"
                        begin="-0.55s"
                        calcMode="spline"
                        dur="1.5s"
                        keySplines=".55, 0, .1, 1; .55, 0, .1, 1"
                        repeatCount="indefinite"
                        values="0; 1; 0"
                      />
                    </path>
                    <path
                      fill="#2a81bf"
                      d="M293.5 163a7.7 7.7 0 0 0-5.3 2L256 195.9L223.8 165a7.8 7.8 0 0 0-10.6 0a6.9 6.9 0 0 0 0 10l37.5 35.8a7.8 7.8 0 0 0 10.6 0l37.5-35.7a6.9 6.9 0 0 0 0-10.1a7.7 7.7 0 0 0-5.3-2.1Z"
                      opacity="0"
                    >
                      <animateTransform
                        attributeName="transform"
                        begin="-0.5s"
                        calcMode="spline"
                        dur="1.5s"
                        keySplines=".55, 0, .1, 1; .55, 0, .1, 1"
                        repeatCount="indefinite"
                        type="translate"
                        values="0 -30; 0 0; 0 30"
                      />
                      <animate
                        attributeName="opacity"
                        begin="-0.5s"
                        calcMode="spline"
                        dur="1.5s"
                        keySplines=".55, 0, .1, 1; .55, 0, .1, 1"
                        repeatCount="indefinite"
                        values="0; 1; 0"
                      />
                    </path>
                  </svg>
                </div>
              </div>
            </div>
            <div
              dir="ltr"
              className="pt-5 lg:mx-0 flex justify-center lg:justify-normal items-center w-full lg:w-1/2"
            >
              <div className=" w-[80vw] sm:w-[70vw] lg:w-[30vw]">
                <div className="rounded-t-md overflow-hidden shadow-xl">
                  <img
                    src={img}
                    alt=""
                    className="w-full object-cover rounded-t-md"
                  />
                </div>

                <div className="bg-white rounded-b-md shadow-2xl p-4 text-center z-10 relative py-8">
                  <h2 className="text-lg font-bold text-center text-gray-800">
                    المحاضرة الأولى
                  </h2>
                  <div className="w-full h-0.5 bg-[#3fc7f3] my-2"></div>
                  <div className="relative rounded px-5 py-2.5 overflow-hidden group bg-[#3fc7f3] hover:bg-gradient-to-r hover:from-imber-500 hover:to-[#3fc7f3] text-white hover:ring-2 hover:ring-offset-2 hover:ring-[#3fc7f3] transition-all ease-out duration-300">
                    <span className="absolute right-0 w-8 h-32 -mt-12 transition-all duration-1000 transform translate-x-12 bg-white opacity-10 rotate-12 group-hover:-translate-x-40 ease"></span>
                    <span className="relative text-base font-semibold">
                      تم شراء الكورس
                    </span>
                  </div>
                </div>
              </div>
            </div>
          </div>
        </section>
        <section className="mx-1 sm:mx-6 rounded-2xl mb-20 shadow-2xl">
          <div className="flex flex-col lg:flex-row min-h-screen">
            <aside className="lg:w-96 w-full bg-gray-100 px-4 py-16 border-t md:border-t-0 flex flex-col rounded-t-2xl lg:rounded-t-none lg:rounded-r-2xl">
              <div className="flex justify-center items-center pb-5 gap-3">
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  width="24"
                  height="24"
                  viewBox="0 0 24 24"
                >
                  <path
                    fill="currentColor"
                    d="M20.27,4.74a4.93,4.93,0,0,1,1.52,4.61,5.32,5.32,0,0,1-4.1,4.51,5.12,5.12,0,0,1-5.2-1.5,5.53,5.53,0,0,0,6.13-1.48A5.66,5.66,0,0,0,20.27,4.74ZM12.32,11.53a5.49,5.49,0,0,0-1.47-6.2A5.57,5.57,0,0,0,4.71,3.72,5.17,5.17,0,0,1,9.53,2.2,5.52,5.52,0,0,1,13.9,6.45,5.28,5.28,0,0,1,12.32,11.53ZM19.2,20.29a4.92,4.92,0,0,1-4.72,1.49,5.32,5.32,0,0,1-4.34-4.05A5.2,5.2,0,0,1,11.6,12.5a5.6,5.6,0,0,0,1.51,6.13A5.63,5.63,0,0,0,19.2,20.29ZM3.79,19.38A5.18,5.18,0,0,1,2.32,14a5.3,5.3,0,0,1,4.59-4,5,5,0,0,1,4.58,1.61,5.55,5.55,0,0,0-6.32,1.69A5.46,5.46,0,0,0,3.79,19.38ZM12.23,12a5.11,5.11,0,0,0,3.66-5,5.75,5.75,0,0,0-3.18-6,5,5,0,0,1,4.42,2.3,5.21,5.21,0,0,1,.24,5.92A5.4,5.4,0,0,1,12.23,12ZM11.76,12a5.18,5.18,0,0,0-3.68,5.09,5.58,5.58,0,0,0,3.19,5.79c-1,.35-2.9-.46-4-1.68A5.51,5.51,0,0,1,11.76,12ZM23,12.63a5.07,5.07,0,0,1-2.35,4.52,5.23,5.23,0,0,1-5.91.2,5.24,5.24,0,0,1-2.67-4.77,5.51,5.51,0,0,0,5.45,3.33A5.52,5.52,0,0,0,23,12.63ZM1,11.23a5,5,0,0,1,2.49-4.5,5.23,5.23,0,0,1,5.81-.06,5.3,5.3,0,0,1,2.61,4.74A5.56,5.56,0,0,0,6.56,8.06,5.71,5.71,0,0,0,1,11.23Z"
                  >
                    <animateTransform
                      attributeName="transform"
                      dur="1.5s"
                      repeatCount="indefinite"
                      type="rotate"
                      values="0 12 12;360 12 12"
                    />
                  </path>
                </svg>

                {/* النص */}
                <div className="text-center font-rubik font-semibold text-3xl">
                  المحاضرة الأولى
                </div>

                {/* أيقونة اليمين */}
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  width="24"
                  height="24"
                  viewBox="0 0 24 24"
                >
                  <path
                    fill="currentColor"
                    d="M20.27,4.74a4.93,4.93,0,0,1,1.52,4.61,5.32,5.32,0,0,1-4.1,4.51,5.12,5.12,0,0,1-5.2-1.5,5.53,5.53,0,0,0,6.13-1.48A5.66,5.66,0,0,0,20.27,4.74ZM12.32,11.53a5.49,5.49,0,0,0-1.47-6.2A5.57,5.57,0,0,0,4.71,3.72,5.17,5.17,0,0,1,9.53,2.2,5.52,5.52,0,0,1,13.9,6.45,5.28,5.28,0,0,1,12.32,11.53ZM19.2,20.29a4.92,4.92,0,0,1-4.72,1.49,5.32,5.32,0,0,1-4.34-4.05A5.2,5.2,0,0,1,11.6,12.5a5.6,5.6,0,0,0,1.51,6.13A5.63,5.63,0,0,0,19.2,20.29ZM3.79,19.38A5.18,5.18,0,0,1,2.32,14a5.3,5.3,0,0,1,4.59-4,5,5,0,0,1,4.58,1.61,5.55,5.55,0,0,0-6.32,1.69A5.46,5.46,0,0,0,3.79,19.38ZM12.23,12a5.11,5.11,0,0,0,3.66-5,5.75,5.75,0,0,0-3.18-6,5,5,0,0,1,4.42,2.3,5.21,5.21,0,0,1,.24,5.92A5.4,5.4,0,0,1,12.23,12ZM11.76,12a5.18,5.18,0,0,0-3.68,5.09,5.58,5.58,0,0,0,3.19,5.79c-1,.35-2.9-.46-4-1.68A5.51,5.51,0,0,1,11.76,12ZM23,12.63a5.07,5.07,0,0,1-2.35,4.52,5.23,5.23,0,0,1-5.91.2,5.24,5.24,0,0,1-2.67-4.77,5.51,5.51,0,0,0,5.45,3.33A5.52,5.52,0,0,0,23,12.63ZM1,11.23a5,5,0,0,1,2.49-4.5,5.23,5.23,0,0,1,5.81-.06,5.3,5.3,0,0,1,2.61,4.74A5.56,5.56,0,0,0,6.56,8.06,5.71,5.71,0,0,0,1,11.23Z"
                  >
                    <animateTransform
                      attributeName="transform"
                      dur="1.5s"
                      repeatCount="indefinite"
                      type="rotate"
                      values="0 12 12;360 12 12"
                    />
                  </path>
                </svg>
              </div>
              <div className="flex justify-center mb-12">
                <div className="bg-gray-300 rounded h-5 w-[90%]">
                  <div className="bg-blue-500 w-[25%] h-5 rounded text-white text-center text-sm transition-all duration-500">
                    25%
                  </div>
                </div>
              </div>{" "}
              <div className="flex flex-col gap-5">
                {sidebarData.map((section) => (
                  <div key={section.path}>
                    <h3 className="text-md font-bold mb-3">{section.title}</h3>

                    <div className="space-y-3">
                      <NavLink
                        to={section.path}
                        end={section.path === "/media"} // مهم لو عندك nested routes
                        className={({ isActive }) =>
                          `cursor-pointer block w-full py-2 px-3 rounded transition-colors ${
                            isActive
                              ? "bg-blue-500 text-white"
                              : "text-black hover:bg-blue-100"
                          }`
                        }
                      >
                        <div className="flex justify-center items-center text-center gap-x-2">
                          <div>{section.icon}</div>
                          <div>{section.btn}</div>
                        </div>
                      </NavLink>
                    </div>
                  </div>
                ))}
              </div>
            </aside>
            <main className="flex-1 p-6 flex bg-white">
              {" "}
              <Outlet />
            </main>
          </div>
        </section>
      </div>
      <Footer />
    </>
  );
}
export default MediaPreview;
